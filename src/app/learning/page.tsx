...
import {
  getLearningContent,
  getUserGptLearningPath,
  getUserProgressInformation,
} from "@/firebase/getData";
import { useUserContext } from "@/context/UserDataContext";
import axios from "axios";
import {
  updateSingleFieldForUser,
  updateGPTLearningPath,
  updateLastQuestionWithPoints,
  updateQuestions,
...
export default function LearningPage() {
  ...
  // @TODO technical problem display on UI
  const [technicalProblem, setTechnicalProblem] = useState(false);
  // const [learningContent, setLearningContent] = useState(false);
  ...
  const handleCodeSubmit = async (answer: string) => {
    ...
    console.log("points", points + data.response.points);
    ...
    // if (points + data.response.points > 250) {
    //   updateAward("250Points");
    //   updateNewAwardSwal(true);
    // }

    if (totalChallenges + 1 === 15) {
      updateAward("500Points");
      updateNewAwardSwal(true);
    }
      ...
      {currentModule && currentChallenge && (
        <>
          <h1 style={styles.Text} className="pb-6 pt-4">
            Course: JavaScript - {capitalizeFirstLetter(currentModule)}
          </h1>
          <LearningView
